...
export default function CreateTripForm({ operatorId }: { operatorId: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="p-6 bg-white rounded-lg shadow-md space-y-4">
      ...
      <input name="title" placeholder="Trip Title (e.g. World Cup Shuttle)" className="w-full border p-2 rounded" required />
      
      <div className="flex gap-4">
        <input name="from_place" placeholder="From (e.g. Accra)" className="w-1/2 border p-2 rounded" required />
        <input name="to_place" placeholder="To (e.g. Kumasi)" className="w-1/2 border p-2 rounded" required />
      </div>
      ...
      <div className="flex gap-4">
        ...
        <div className="w-1/2">
          <label className="text-sm font-bold text-green-600">Scout Reward (GH₵)</label>
          <input name="scout_bounty" type="number" className="w-full border-2 border-green-500 p-2 rounded" required />
        </div>
      </div>

      <input name="departure_date" type="datetime-local" className="w-full border p-2 rounded" required />
